...
FROM (SELECT board.user_id                       AS user_id,
             DATE_TRUNC('day', board.created_at) AS created_at,
      ...
      GROUP BY board.user_id, DATE_TRUNC('day', board.created_at)

      UNION ALL

      SELECT comment.user_id                       AS user_id,
             DATE_TRUNC('day', comment.created_at) AS created_at,
             0                                     AS board_count,
      ...
      GROUP BY comment.user_id, DATE_TRUNC('day', comment.created_at)

      UNION ALL

      SELECT community.user_id                       AS user_id,
             DATE_TRUNC('day', community.created_at) AS created_at,
             0                                       AS board_count,
             0                                       AS comment_count,
      ...
      GROUP BY community.user_id, DATE_TRUNC('day', community.created_at)

      UNION ALL

      SELECT message.user_id                       AS user_id,
             DATE_TRUNC('day', message.created_at) AS created_at,
             0                                     AS board_count,
             0                                     AS comment_count,
             0                                     AS community_count,
      ...
      GROUP BY message.user_id, DATE_TRUNC('day', message.created_at)

      UNION ALL

      SELECT note.user_id                       AS user_id,
             DATE_TRUNC('day', note.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY note.user_id, DATE_TRUNC('day', note.created_at)

      UNION ALL

      SELECT post.user_id                       AS user_id,
             DATE_TRUNC('day', post.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY post.user_id, DATE_TRUNC('day', post.created_at)

      UNION ALL

      SELECT task.user_id                       AS user_id,
             DATE_TRUNC('day', task.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY task.user_id, DATE_TRUNC('day', task.created_at)

      UNION ALL

      SELECT test.user_id                       AS user_id,
             DATE_TRUNC('day', test.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY test.user_id, DATE_TRUNC('day', test.created_at)) AS subquery
WHERE user_id IS NOT NULL
   OR created_at IS NOT NULL
GROUP BY user_id, created_at;
  `,
})
export class UserSummaryByDay {
  @ViewColumn()
  user_id: string;
