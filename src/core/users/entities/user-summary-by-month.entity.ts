...
@ViewEntity({
  name: 'user_summary_view_by_month',
  expression: `
SELECT user_id,
       created_at,
...
FROM (SELECT board.user_id                       AS user_id,
             DATE_TRUNC('month', board.created_at) AS created_at,
      ...
      GROUP BY board.user_id, DATE_TRUNC('month', board.created_at)

      UNION ALL

      SELECT comment.user_id                       AS user_id,
             DATE_TRUNC('month', comment.created_at) AS created_at,
             0                                     AS board_count,
      ...
      GROUP BY comment.user_id, DATE_TRUNC('month', comment.created_at)

      UNION ALL

      SELECT community.user_id                       AS user_id,
             DATE_TRUNC('month', community.created_at) AS created_at,
             0                                       AS board_count,
             0                                       AS comment_count,
      ...
      GROUP BY community.user_id, DATE_TRUNC('month', community.created_at)

      UNION ALL

      SELECT message.user_id                       AS user_id,
             DATE_TRUNC('month', message.created_at) AS created_at,
             0                                     AS board_count,
             0                                     AS comment_count,
             0                                     AS community_count,
      ...
      GROUP BY message.user_id, DATE_TRUNC('month', message.created_at)

      UNION ALL

      SELECT note.user_id                       AS user_id,
             DATE_TRUNC('month', note.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY note.user_id, DATE_TRUNC('month', note.created_at)

      UNION ALL

      SELECT post.user_id                       AS user_id,
             DATE_TRUNC('month', post.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY post.user_id, DATE_TRUNC('month', post.created_at)

      UNION ALL

      SELECT task.user_id                       AS user_id,
             DATE_TRUNC('month', task.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY task.user_id, DATE_TRUNC('month', task.created_at)

      UNION ALL

      SELECT test.user_id                       AS user_id,
             DATE_TRUNC('month', test.created_at) AS created_at,
             0                                  AS board_count,
             0                                  AS comment_count,
             0                                  AS community_count,
             0                                  AS message_count,
      ...
      GROUP BY test.user_id, DATE_TRUNC('month', test.created_at)) AS subquery
WHERE user_id IS NOT NULL
   OR created_at IS NOT NULL
GROUP BY user_id, created_at;
  `,
})
export class UserSummaryByMonth {
  @ViewColumn()
  user_id: string;
